...
const userData = data?.me || {};
  ...
  // create function that accepts the blog's mongo _id value as param and deletes the blog from the database
    ...
    try {
        const response = await removeBlogId({
        variables: { blogId},
        });

      // upon success, remove blog's id from localStorage
        removeBlogId(blogId);
    } catch (err) {
        console.error(err);
    }
  ...
  // if data isn't here yet, say so
    ...
    <>
        ...
        <h2>
            ...
            ? `Viewing ${userData.savedBlog.length} saved ${userData.savedBlog.length === 1 ? 'blog' : 'blogs'}:`
